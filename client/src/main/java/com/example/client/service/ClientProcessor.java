...
public class ClientProcessor {
    ...
    public List<Object> split(byte[] fileData, int chunkSize, String fileName) {
        ...
        if(fileData.length==0){
            ...
            System.arraycopy(fileData, 0, chunk, 0, chunk.length);
            chunkToChunkMasters.add(new ChunkToChunkMaster(uuid, fileName, 1));
            chunkToChunkServers.add(new ChunkToChunkServer(uuid, new String(chunk, StandardCharsets.UTF_8), fileName, 1));
